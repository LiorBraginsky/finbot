"""The seed drift guard: `categories` is seeded by the migration, and its
single source of truth is `finbot.core.categories.catalog.CATALOG`
...
async def test_seeded_categories_match_the_catalog_exactly(db_session: AsyncSession) -> None:
    ...
    expected = sorted(
        ((c.slug, c.emoji, True, "active") for c in CATALOG),
...
async def test_seeded_categories_have_thirteen_rows(db_session: AsyncSession) -> None:
