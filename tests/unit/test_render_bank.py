...
def test_clean_screenshot_reports_only_the_header_and_the_written_count() -> None:
    ...
    note = render_bank_note(summary, anchor=_ANCHOR)
...
def test_every_skip_reason_at_once_matches_the_planned_wording_exactly() -> None:
    ...
    plan = BankPlan(
        anchor=_ANCHOR,
        writes=(_write(), _write(), _write(), _write()),
        skipped_by_kind={
    ...
    summary = BankSummary(
        plan=plan, duplicates=1, manual_collisions=(_collision("кава", "150.00"),)
    )
...
def test_wholly_empty_plan_is_not_a_bank_feed_reply() -> None:
    ...
    assert render_bank_note(summary, anchor=_ANCHOR) == NOT_A_BANK_FEED_REPLY
...
def test_note_stays_well_under_telegrams_message_limit_for_a_worst_case_feed() -> None:
    ...
    summary = BankSummary(plan=plan, duplicates=20, manual_collisions=collisions)
