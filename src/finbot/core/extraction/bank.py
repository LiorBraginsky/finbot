...
def bank_txn_key(*, occurred_at: date, time: str | None, amount: Decimal) -> str:
    ...
    double-count money, the worst outcome this stage can produce. `time=None`
    and `time=""` collapse to the same key: neither carries information the
    other does not.
    """
    return f"{occurred_at.isoformat()}|{time or ''}|{amount:.2f}"
...
def plan_writes(result: BankExtractionResult, *, anchor: date) -> BankPlan:
    ...
    for row in result.rows:
        if row.kind is BankRowKind.UNCLASSIFIED:
            unclassified += 1
            continue
        if row.kind in _SKIPPED_KINDS:
            skipped_by_kind[row.kind] = skipped_by_kind.get(row.kind, 0) + 1
            continue
        # Only BankRowKind.EXPENSE remains.
        ...
        try:
            ...
            # to_amount also enforces the upper bound (core.money.MAX_AMOUNT)
            # the row-level `amount <= 0` check above does not — both land in
            # the same counter, since either way the amount was unusable.
            bad_amount += 1
            continue
