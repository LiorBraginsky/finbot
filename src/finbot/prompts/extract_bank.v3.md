...
9. `is_transaction_feed` is `false` when the image is not a bank transaction feed at all
   ...
   `rows` must be empty.
10. `suggested_category` is a proposal for a category that does not exist yet. Set it
   **only** when `category` is `other` *and* you can name what this row really is in
